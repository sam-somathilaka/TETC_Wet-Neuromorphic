...
def GenerateInputCocktail(FullGeneList, Gene1, Gene2, G1Cons, G2Cons, MinCons):

    Cocktail = []

    for gene in FullGeneList:
        if gene in Gene1:
            Cocktail.append(G1Cons)
        elif gene in Gene2:
            Cocktail.append(G2Cons)
        else:
...
InputGeneList = ['b3067', 'b3357']

FinalDF = pd.DataFrame(index=FullInputGenes)

for G1Cons in np.arange(0, 5.1, 0.2):
    for G2Cons in np.arange(0, 5.1, 0.2):
        for ite in range(10):
            G1Cons = round(G1Cons, 1)
            G2Cons = round(G2Cons, 1)
            print('Cons', G1Cons, G2Cons, 'ite', ite)
            InputCocktail = GenerateInputCocktail(FullInputGenes, InputGeneList[0], InputGeneList[1], G1Cons, G2Cons, 2)
            # print(FullInputGenes)
            # print(InputCocktail)

            Output = CompleteCycles(WeightMatrix, InputCocktail, BiasArray, 10)
#
            FinalDF['Cons'+str(G1Cons)+'_'+str(G2Cons)+'Ite'+str(ite)] = Output
#
# print(FinalDF.head())
FinalDF.to_csv('Results/MultiReg.csv')
